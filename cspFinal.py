...
def process_file(f, delim, colors, nodes, graph, arc, ledger): #f is file input
    ...
    # populate the ledger(maintains Arc Consistency)
    for i in graph.keys():
...
class mapcolor:
    def __init__(self, domain, values, graph, arcs, ledger):
        ...
        self.ledger = ledger #Color consistency


def AC_next(solution, csp):
    # Done with Most Restrained Variable
    remaining_vars = defaultdict(list) #All the values not in the solution matrix
    for value in csp.values:
        if value not in solution.keys():
            remaining_vars[value] = len(csp.ledger[value]) #Assign remaining_vars to # of colors left
            mrv = min(remaining_vars.keys(), key=(lambda k: remaining_vars[k])) #value with least amount of remaining colors
    return mrv
...
def AC_ledger(solution, color, xj, csp): 
    for xi in csp.graph[xj]: #all xi from (:,xj)
...
def consistent(color, cur_xi, solution, csp): #local consistency for individual value
...
#values are like depth, colors are like breadth
def backtrack(solution, csp):
    ...
    for color in csp.ledger[cur_xi]:  # run backtrack on each possible color for Xi
        ...
        if AC_ledger(solution, color, cur_xi, csp):
...
def reassign(solution, csp):
    ...
    solution[max_conflict] = min(conflicts.keys(),
                                 key=(lambda k: conflicts[k]))  # reassign max_conflict to least conflicted color
